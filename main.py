...
# منابع تقویت شده
SMART_SOURCES = {
    "fars": ["shiraz_online", "akhbarshiraz", "asrshiraz", "shiraz_it"],
    "hormozgan": ["bndonline", "hormozgan_online", "akhbar_hormozgan", "hormozgan_today", "bandar_news"]
}
...
def init_db():
    try:
        ...
        # اصلاح ساختار جدول برای نسخه های قدیمی (Migration)
        try: cur.execute("ALTER TABLE msg_logs ADD COLUMN type TEXT")
        except: conn.rollback()
        conn.commit(); cur.close(); conn.close()
        logging.info("✅ Database structure is up to date.")
    except Exception as e: logging.error(f"DB Error: {e}")

def ai_classify(text):
    if not GEMINI_API_KEY: return "سایر اخبار"
    prompt = f"فقط نام دسته را بنویس: {', '.join(CATEGORIES)}\n\nخبر:\n{text[:400]}"
    try:
...
def run_check():
    ...
    for p_id, channel in prov_map.items():
        logging.info(f"Checking sources for: {p_id.upper()}")
        sources = SMART_SOURCES[p_id]
        for src in sources:
            posts = scrape_tg(src)
            logging.info(f"@{src}: {len(posts)} posts found.")
            for p in posts:
                ...
                if not cur.fetchone():
                    ...
                    try:
                        ...
                        if p['type'] == "video" and p['media']:
                            ...
                            res = requests.post(tg_url+"sendVideo", data={"chat_id":channel, "caption":caption, "parse_mode":"HTML", "reply_markup":json.dumps(kb)}, files={"video":("v.mp4", video_data)})
                        ...
                        if res and res.status_code == 200:
                            ...
                            cur.execute("INSERT INTO msg_logs (hash, channel_id, msg_id, title, prov, type) VALUES (%s,%s,%s,%s,%s,%s)", (h, channel, str(m_id), p['text'][:300], p_id, p['type']))
                            conn.commit()
...
def webhook():
    ...
    if "callback_query" in data:
        ...
        if row:
            ...
            prompt = f"خبر زیر را با واژگان انقلابی بازنویسی کن:\n{title}"
...
@app.route('/')
def home(): return "Bot Online v18"
